...
Model::Model(const std::string &name, std::shared_ptr<Body> body_ptr,
             const std::string &meta_file, float sphere_radius, int n_divides,
...
void Model::set_meta_file(const std::string &filename) {
  meta_file_ = filename;
  set_up_ = false;
}
...
bool Model::GenerateModel() {
  ...
  // requirements
  std::shared_ptr<RendererGeometry> renderer_geometry_ptr = std::make_shared<RendererGeometry>("rg");
...
bool Model::SaveModel() const {
  ...
  int n_template_views = template_views_.size();
